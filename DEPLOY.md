...
## 4. Volume (постоянное хранилище для БД и файлов)
...
3. **Mount Path**: `/app/data` (SQLite) и/или `/data/uploads` (файлы).

После этого SQLite-база (`am_hub.db`) будет жить вечно.

### Аплоады файлов (`routers/files.py`)

Файлы, которые менеджеры загружают в своём кабинете, хранятся в каталоге
`FILES_STORAGE_DIR` (по умолчанию — `/data/uploads`, предполагается смонтированный
Railway Volume). Если `/data/uploads` недоступен (локальная разработка) —
приложение автоматически падает на `/tmp/amhub_files`, но тогда файлы
**не переживут рестарт**. Для прода:

1. В проекте → **+ New** → **Volume**
2. **Mount Path**: `/data/uploads`
3. Переменная окружения `FILES_STORAGE_DIR=/data/uploads` (опционально —
